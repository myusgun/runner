...
class Property:
	...
	# Task에서 raise 되었을 때, 계속 반복할지 여부
	contOnErr = True

	# 사용할 플러그인 목록
	plugins   = ['plugin_example']

	# 가상화되는 경우, 공유 폴더 마운트 경로 (자동 설정)
	# 서버의 다른 드라이브들에 접근할 수 있는 root 경로
	mount     = ''

	# 가상화되는 경우, Task 실행 후 자동 백업할 대상
	backup    = []

# Task의 "동작"을 정의하기 위한 action() 함수
...
def action(do):
	...
	mail_form = {
		...
		'to': ['charlie@unsigned.kr', 'dave@unsigned.kr'],
